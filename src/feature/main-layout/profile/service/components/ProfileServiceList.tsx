...
const ProfileServiceList = ({ services }: { services: any[] }) => {
    ...
    <div className=" h-full">
      <div className="flex items-center justify-between mb-6">
        ...
        <button
          ...
          className="flex items-center gap-2 px-5 py-2.5 bg-[#055e6e] text-white rounded-lg text-sm font-medium hover:bg-[#044a57] transition-colors shadow-sm"
        >
          <Plus size={18} />
        ...
        {services?.length === 0 ? (
          ...
          services?.map((service, index) => (
            <ProfileServiceCard
              ...
              image={
                service.serviceData.image?.startsWith("http")
                  ? service.serviceData.image
                  : `${imgUrl}${service.serviceData.image}`
              }
              onEdit={() => {
                setOpen(true);
