...
import {
  SubscriptionPackage,
  UserSubscription,
} from "./components/subscriptionType";
import { myFetch } from "../../../../../helpers/myFetch";
import { Empty } from "antd";

const Subscription = () => {
  const [isOpen, setIsOpen] = React.useState(false);
  const [mySubscription, setMySubscription] = React.useState<UserSubscription>(
    {} as UserSubscription,
  );
  useEffect(() => {
  ...
  return (
    <div className="w-full">
      <div className="flex justify-between items-center mb-8">
        ...
        <button
          className="px-6 py-2.5 rounded-lg bg-primary text-white text-sm font-medium hover:bg-[#044a57] transition-colors"
          onClick={() => setIsOpen(true)}
        >
          Buy Package
        </button>
      </div>
      {!mySubscription?.package ? (
        <Empty description="No Subscription Found" />
      ) : (
        <SubscriptionCard
          subscription={mySubscription?.package}
          mySubscription={true}
        />
      )}
