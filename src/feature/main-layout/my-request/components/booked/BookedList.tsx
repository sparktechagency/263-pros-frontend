      ...
      }),
      {
        ...
        success: async (res) => {
          if (res?.success) {
            ...
            return res?.message || "Review submitted successfully";
          }
          throw new Error(res?.message || "Review submission failed");
        },
        error: (err) => err.message || "Error submitting review",
      },
    );
  };

  const completeBooking = async (id: string, status: string) => {
    try {
      toast.promise(
